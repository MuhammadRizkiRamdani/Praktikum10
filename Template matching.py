...
einstein = cv2.imread('riram3.jpg')
einstein = cv2.cvtColor(einstein, cv2.COLOR_BGR2RGB) # simpan dengan nama yang sama = ditumpuk
# panggil dan konversi warna agar sesuai dengan Matplotlib
solvay = cv2.imread('riram2.jpg')
solvay = cv2.cvtColor(solvay, cv2.COLOR_BGR2RGB)
plt.subplot(121),plt.imshow(einstein), plt.title('Riram')
plt.subplot(122),plt.imshow(solvay), plt.title('Barengan')
plt.show()

img = cv2.imread('riram2.jpg',0)
img2 = img.copy()
template = cv2.imread('riram3.jpg',0)
